...
`WITHDRAW_SOL` / `WITHDRAW_TOKEN`-style intents; the system validates, resolves,
policy-checks, builds the domain-separated signing payload, and tracks intent lifecycle.
...
- PRD_v5 §5 (withdrawals); PRD_v4 §5.4 (Intent Service), §5.5 (Policy Service), §13
  (transaction flow), §25 (intent security), §16 (Wallet API)

## Repository Context

- `intents` + `transactions` tables from 001; account resolution from 002; on-chain nonce is
  read via the adapter (006) — this task consumes, not implements, the adapter interface.
...
- Intent types for V1: `WITHDRAW_SOL`, `WITHDRAW_TOKEN` (map to the program's withdraw
  instructions; PRD_v5 §5). Top-ups are plain transfers built client-side by 006/009 —
  no intent needed (PRD_v5 §4).
...
- The Solana build/sign/submit mechanics (task 006).
- Policy types beyond §5.5 (no spend limits/session keys — future).
- SDK surface (task 009).

## Dependencies

- 001 (tables), 002 (accounts), 006 (adapter interface).

## Acceptance Criteria

- `WITHDRAW_SOL` intent end-to-end against a mocked adapter in integration tests: create →
...
- The payload the user signs must be byte-for-byte what the program verifies (with 004/006 —
  cross-tested in 011).

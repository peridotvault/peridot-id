...
> **Solana chains/RPC/program id are not env.** They live in the chain registry
> (DB), seeded by `pnpm db:seed` and edited in the workspace admin
> (`GET /v1/chains`, `POST /v1/admin/chains`). RPC API keys, if any, must be
> public or proxied — the registry is served to clients as-is.

### Fiat (DOKU Checkout + internal ledger)

Fiat is one namespace: **DOKU Checkout is money-in only** (no Sub-Account); every
balance lives on the internal fiat ledger. See [Fiat & payments](/docs/fiat).

...
1. Call `POST /v1/auth/login` (or use the SDK) to get the Google authorization URL.
   First-time users pass a `handle` (check `GET /v1/auth/pid/available?handle=...`
   first) — it becomes the permanent `<handle>@pid`. Returning users omit it.
...
On a first-time sign-in the API creates a new identity (`<handle>@pid`), a profile
...
- A non-null `email` is **unique across accounts**: one email = one PID. If a new login
  presents an email already associated with another account, it is rejected with `409`
  (`Email sudah terhubung dengan akun lain`) — accounts are never merged, moved, or minted
  twice (see [Errors](/docs/errors)).

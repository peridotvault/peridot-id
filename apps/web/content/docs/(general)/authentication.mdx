...
3. Google redirects to `GET /v1/auth/google/callback?code=...`. The API exchanges the
   code and resolves the credential.
4. Returning credential → session cookies are issued and the browser lands on
   `CLIENT_SUCCESS_URL` with a logged-in session. New credential → the PID picker
   (see below).

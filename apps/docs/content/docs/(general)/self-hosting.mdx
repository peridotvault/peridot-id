...
1. **Supabase** — create a project, then run migrations against it (use the session pooler,
   ...
   DATABASE_URL="postgresql://postgres.<ref>:<password>@aws-0-<region>.pooler.supabase.com:5432/postgres" \
     pnpm --filter @antigane/api db:deploy
   ```
...
pnpm build
pnpm --filter @antigane/api db:deploy
pnpm --filter @antigane/api start
```
...
2. Deploy with `pnpm --filter @antigane/api db:deploy` (same flow as the Supabase steps above).

/// <reference lib="dom" />
// Raw-WebGL port of the R3F AsciiRidges canvas (same shaders, same uniforms,
// same motion). Zero dependencies: three/fiber only ever provided scaffolding.
// Pixels match the R3F version by construction — including its quirks:
// sRGB->linear tint (three ColorManagement), flipY atlas, CSS-px mouse vs
// device-px resolution mapping.
...
const AsciiRidgesCanvas: React.FC<AsciiRidgesProps> = (props) => {
  const {
    characters = " .:-+*=%@#",
    color = "#87ee83",
    invert = false,
  ...
  useEffect(() => {
    ...
    const safeChars =
      propsRef.current.characters && propsRef.current.characters.length > 0
        ? propsRef.current.characters
        : " ";
    ...
    const draw = (p: AsciiRidgesProps) => {
      const chars =
        p.characters && p.characters.length > 0 ? p.characters : " ";
      if (chars !== atlasChars) {
        atlasChars = chars;
        uploadAtlas(chars);
      }
...
export default AsciiRidgesCanvas;
